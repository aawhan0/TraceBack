...
def _response(job: InvestigationJob) -> JobResponse:
    return JobResponse(
        job_id=job.job_id, scenario_id=job.scenario_id, mode=job.mode, model=job.model,
        status=job.status, created_at=job.created_at.isoformat(),
        started_at=job.started_at.isoformat() if job.started_at else None,
        completed_at=job.completed_at.isoformat() if job.completed_at else None,
        run_id=job.run_id, execution_id=job.execution_id, trace_id=job.trace_id,
        error_type=job.error_type, error_message=job.error_message,
    )

@router.post("", response_model=JobResponse, status_code=202)
def create_job(request: JobRequest, background_tasks: BackgroundTasks) -> JobResponse:
    try:
        get_scenario(request.scenario_id)
    except KeyError as exc:
        raise HTTPException(status_code=404, detail="Scenario not found") from exc
    try:
        job = _store.create(request.scenario_id, request.mode, request.model)
