...
export default function HomePage() {
  ...
  return <div className="space-y-6">
    ...
    <nav className="flex gap-2 border-b border-border pb-2">{([['investigate', Search, 'Investigate'], ['experiments', FlaskConical, 'Experiments'], ['history', History, 'History']] as const).map(([key, Icon, label]) => <Button key={key} type="button" size="sm" variant={view === key ? 'secondary' : 'ghost'} onClick={() => setView(key)}><Icon />{label}</Button>)}</nav>
