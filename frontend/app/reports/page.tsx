...
export default function ReportsPage() {
  ...
  useEffect(() => {
    fetch(`${API}/runs?limit=50`)
      .then(async (response) => {
        if (!response.ok) {
          throw new Error('Unable to load investigation runs')
        }
  ...
  function exportJson() {
    ...
    download(
      `traceback-${selected.run_id}.json`,
  ...
  function exportMarkdown() {
    ...
    const lines = [
      ...
      `- **Duration:** ${formatNumber(selected.duration_ms)} ms`,
      ...
      `| Evidence recall | ${formatPercent(selected.evidence_recall)} |`,
      `| Evidence precision | ${formatPercent(selected.evidence_precision)} |`,
  ...
  return (
    <div className="space-y-6">
      <PageHeader
        title="Reports"
        description="Export a persisted investigation run as a portable report."
      />
      ...
      {loading ? (
        ...
        <div className="rounded-lg border border-dashed p-10 text-center text-sm text-muted-foreground">
          No investigation runs are available to export.
        </div>
      ) : (
        <div className="grid gap-6 overflow-hidden lg:h-[calc(100vh-9rem)] lg:min-h-0 lg:grid-cols-[280px_minmax(0,1fr)]">
          <section className="flex min-h-0 flex-col overflow-hidden rounded-lg border bg-card p-3">
            <div className="px-2 pb-3 text-sm font-semibold">Select run</div>
                ...
                {runs.map((run) => (
                  <button
                    ...
                    className={cn(
                      ...
                      selected?.run_id === run.run_id && 'bg-accent',
                    )}
                  >
              ...
              <>
                  ...
                  <div className="flex flex-wrap items-start justify-between gap-4">
                    <div>
                      ...
                      <p className="mt-1 text-sm text-muted-foreground">
                        {selected.run_id} - {selected.scenario_id}
                      </p>
                    </div>
                ...
                <div className="grid gap-4 sm:grid-cols-2">
                  ...
                  <div className="rounded-lg border bg-card p-5">
                    ...
                    <p className="mt-2 text-sm leading-6">
                      {selected.diagnosis?.recommended_action ||
                        'Not available in this run.'}
                    </p>
                  </div>
                </div>
                ...
                <div className="rounded-lg border bg-card p-5">
                  ...
                  <div className="grid gap-3 sm:grid-cols-5">
                    {[
                      [
                        'Root cause',
                        selected.root_cause_match ? 'Pass' : 'Fail',
                      ],
                      ['Recall', formatPercent(selected.evidence_recall)],
                      [
                        'Precision',
                        formatPercent(selected.evidence_precision),
                      ],
                      ['Confidence', formatPercent(selected.confidence)],
                      [
                        'Action',
                        selected.action_present ? 'Pass' : 'Fail',
                      ],
                    ].map(([label, value]) => (
                      <div
                        key={label}
