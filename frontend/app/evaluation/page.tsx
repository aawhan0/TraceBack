...
import { BadgeCheck, CircleAlert, LoaderCircle, XCircle } from 'lucide-react'
...
export default function EvaluationPage() {
  ...
  useEffect(() => {
    fetch(`${API}/runs?limit=50`)
      .then(async (response) => {
        if (!response.ok) throw new Error('Failed to load investigation runs')
            ...
            {detailLoading ? (
              ...
              <>
                ...
                <div className="rounded-lg border bg-card p-5 text-sm text-muted-foreground">
                  <span className="font-medium text-foreground">Evaluation rule:</span> an investigation passes only when root-cause match, evidence recall, evidence precision, confidence validity, and recommended-action presence all satisfy the evaluator. Recall and precision must each equal 100%.
                </div>
              </>
            ) : null}
          </section>
