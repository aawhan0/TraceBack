...
export default function HistoryPage() {
  ...
  const columns: ColumnDef<Run>[] = [
    ...
    { key: 'created_at', header: 'Created', sortable: true, render: (run) => <span className="whitespace-nowrap text-muted-foreground">{run.created_at ? new Date(String(run.created_at)).toLocaleString() : '—'}</span> },
  ]
  return <div className="space-y-6"><PageHeader title="History" description="Previous diagnosis runs and their evaluation signals." />{runs.length ? <DataTable columns={columns} data={runs} searchKeys={['scenario_id', 'mode', 'provider']} searchPlaceholder="Search investigations…" pageSize={10} /> : <EmptyState icon={HistoryIcon} title="No investigations yet" description="Run an investigation to start building the diagnosis history." />}</div>
}
