...
export default function InvestigationDetailPage({
  ...
  useEffect(() => {
    void Promise.all([
      fetch(`${API}/runs/${encodeURIComponent(run_id)}`).then(async (response) => {
      ...
      .then(([runData, scenarios]) => {
        setRun(runData)
        setScenario(
          scenarios.find((item) => item.id === runData.scenario_id) ?? null,
        )
      })
      .catch((item) =>
        setError(
  ...
  return (
    <div className="space-y-6">
      <PageHeader
        ...
        description={scenario?.title || run.scenario_id}
      />
      ...
      <div className="flex flex-wrap items-center justify-between gap-3">
        ...
        <span
          className={
            run.passed
              ? 'inline-flex items-center gap-2 text-sm font-medium text-emerald-600'
      ...
      <div className="grid gap-4 lg:grid-cols-2">
        <section className="rounded-lg border bg-card p-5">
          ...
          <p className="mt-2 text-sm leading-6">
            {run.diagnosis?.root_cause || 'Not available.'}
          </p>
        </section>
        ...
        <section className="rounded-lg border bg-card p-5">
          ...
          <p className="mt-2 text-sm leading-6">
            {run.diagnosis?.recommended_action || 'Not available.'}
          </p>
        </section>
      </div>

      <section className="rounded-lg border bg-card p-5">
        <h2 className="text-base font-semibold">Evidence explorer</h2>
        <p className="mt-1 text-sm text-muted-foreground">
          Evidence selected by this diagnosis, alongside the remaining incident
          context.
        </p>
          ...
          {(scenario?.evidence ?? []).map((item) => (
            <article key={item.id} className="rounded-md border p-4">
              <div className="flex flex-wrap items-center justify-between gap-2">
                ...
                >
                  {selected.has(item.id) ? 'Selected' : 'Unused'}
                </span>
              </div>
      ...
      <div className="text-xs text-muted-foreground">
        Run ID: <code>{run.run_id}</code> · Created{' '}
