...
def _payload(scenario_id: str) -> dict[str, object]:
    return {
        ...
        "evidence": [
            {"id": "ev-payment-001", "source": "api", "kind": "logs", "content": "payment-service: gateway timeout"},
            {"id": "ev-payment-002", "source": "gateway", "kind": "metrics", "content": "gateway timeout rate increased"},
        ],
...
def test_custom_scenario_can_be_investigated(monkeypatch, tmp_path):
    ...
    response = client.post("/investigations", json={"scenario_id": scenario_id, "mode": "baseline"})
